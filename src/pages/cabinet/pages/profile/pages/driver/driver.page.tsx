...
const bankOptions = [
  ...
  { value: "2", label: "Альфа" },
  { value: "3", label: "Синара" },
  { value: "4", label: "ВТБ" },
];

const mapNumberToBankOptions = (
  num: number
...
export const DriverPage = () => {
  ...
  const [isPaymentInfoAlreadyExist, setPaymentInfoAlreadyExist] =
    useState(false);

  const [phoneToTransfer, setPhoneToTransfer] = useState<string | null>(null);
  const [bank, setBank] = useState(0);
  const [transports, setTransports] = useState<
    Partial<GetTransportsResponseDto["transports"]>
  >([]);
  const [newTransports, setNewTransports] = useState<
    Partial<GetTransportsItemResponseDto>[]
  >([]);
  ...
  const saveState = async () => {
    ...
    if (!isPaymentInfoAlreadyExist) {
      ...
      try {
        await api(() =>
          setProfileDataApi(accessTokenGetter(), {
  ...
  const onTransportSave = async (
    transport: Omit<GetTransportsResponseDto["transports"][number], "id">
  ) => {
      ...
      <div>
        ...
        <div className={styles.userPropertyDiv}>
          ...
          <div className={styles.userPropertyValueDiv}>
            <InputComponent
              onChange={setPhoneToTransfer}
              isReadonly={isPaymentInfoAlreadyExist}
              maxLength={11}
